...
type
  TFrmWhosApp = class(TForm)
    ...
    btnEncode2: TButton;
    memHelp1: TMemo;
    btnUnderstood1: TButton;
    memHelp2: TMemo;
    btnUnderstood2: TButton;
    lblHeading1: TLabel;
    lblHeading2: TLabel;
    ...
    procedure btnUnderstood1Click(Sender: TObject);
    ...
    procedure btnUnderstood2Click(Sender: TObject);
...
begin
 ...
 btnUnderstood1.Visible := True;
 btnUnderstood1.Enabled := True;
end;

procedure TFrmWhosApp.btnHelp2Click(Sender: TObject);
begin
 memHelp2.Visible := True;
 btnUnderstood2.Enabled := true;
 btnUnderstood2.Visible := true;
end;
...
begin
 ...
 if lbl1Sent1.Caption = '' then
  ...
  if lbl2Received1.Caption = '' then
   ...
   //clearing the edit box to make space for new message
   edtMessage1.Clear;
   edtMessage1.SetFocus;
   bMessageEncoded1       := False;
   btnSend1.Enabled       := False;
   edtMessage1.ReadOnly   := False;
   lblCharacters1.Caption := '0/60 Characters'
...
begin
 ...
 if lbl2Sent1.Caption = '' then
  ...
  if lbl1Received1.Caption = '' then
   ...
   //clearing the edit box to make space for new messages
   edtMessage2.Clear;
   edtMessage2.SetFocus;
   bMessageEncoded2       := False;
   btnSend2.Enabled       := False;
   edtMessage2.ReadOnly   := False;
   lblCharacters2.Caption := '0/60 Characters'
 End
 Else
  Begin
...
procedure TFrmWhosApp.btnUnderstood1Click(Sender: TObject);
begin
 memHelp1.Visible       := False;
 btnUnderstood1.Visible := False;
 btnUnderstood1.Enabled := False;
end;

procedure TFrmWhosApp.btnUnderstood2Click(Sender: TObject);
begin
 memHelp2.Visible       := False;
 btnUnderstood2.Visible := False;
 btnUnderstood2.Enabled := False;
end;
 ...
 Begin
  ...
  if (sMessage <> '') then
    Begin
      for i := 1 to Length(sMessage) do
        Begin
          if NOT(sMessage[i] IN ['a'..'z', 'A'..'Z', '.', ',', '?', '!', ' ']) then
            Begin
              Delete(sMessage, i, 1);
              bValid := True;
            End;
    ...
    if bValid then
      Begin
        Showmessage('Message may only consist of letters, commas, full stops, question marks, exclaimation marks and spaces');
      End;
