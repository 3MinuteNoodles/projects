...
uses
  ...
  Vcl.ExtCtrls, dmEducation_u, Login_u, System.UITypes, Vcl.ComCtrls;
...
procedure TfrmtTest.btnHelpClick(Sender: TObject);
begin
  redHelp.show;
  btnUnderstood.show;
end;
...
begin
    ...
    if bValid = False then     //error message
      ...
      AssignFile(tTest, edtTitle.text + 'N.txt');
      if fileExists(edtTitle.text + 'N.txt') then
    ...
    begin
     ...
     With dmEducation do
      begin
       ...
       tblTests['TextFile'] := edtTitle.text + 'T.txt';
       tblTests.post;
       bIncomplete := false;
      end;
...
procedure TfrmtTest.btnUnderstoodClick(Sender: TObject);
begin
 redHelp.hide;
 btnUnderstood.hide;
end;
