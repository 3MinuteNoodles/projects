...
uses
  ...
  Vcl.ExtCtrls, dmdbWhysapp_u, Keywords_u, Vcl.Buttons;
...
procedure TfrmUsers.btnHelpClick(Sender: TObject);
begin
  redhelpUsers.ReadOnly := True;
  redHelpUsers.Visible  := True;
  redHelpusers.Enabled  := True;
...
procedure TfrmUsers.btnNotHelpClick(Sender: TObject);
begin
  redHelpUsers.Visible  := False;
  redHelpusers.Enabled  := False;
...
var
  bFound: boolean;
begin
 if pnlUserID.caption = 'UserID:' then
   begin
     Showmessage('Please select a user to disable in the select user combo box');
   end;
