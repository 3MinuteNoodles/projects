...
var
  sNewKeyword, sKEyword, shex, sCheckKeyword : string;
  i, iWeight : integer;
  bFound, bInvalid : boolean;
begin
  ...
  if sKeyword = '' then
    Begin
      Showmessage('Keyword can not be blank');
      Exit;
    End;

  for i := 1 to iNumKeywords do
    begin
      sCheckKeyword := Decryptkeyword(ArrKeywords[i]);

      if Uppercase(sKeyword) = Uppercase(COPY(sCheckKeyword, 1, pos('¡', sCheckKeyword) - 1)) then
        Begin
          Showmessage('This keyword already exists');
          Exit;
        End;
  ...
  if bFOund then
   ...
   //Gets the weight for the keyword
  iWeight := -1;
  while iWeight = -1 do
    Begin
      bInvalid := false;

      try    //This gives an error in debug mode but not normal mode ( still works correctly in both
      ...
      if ((iWeight < 1) or (iWeight > 50)) and NOT(bInvalid) then
        begin
          Showmessage('Please enter a number between 1 and 50');
          iWeight := -1;
        end;

      if mrYes <> MessageDlg('Do you still want to continue entering the keyword?', mtConfirmation, [mbYes, mbNO], 0) then
        Begin
...
var
  ...
  sKeyword : string;
begin
  if cmbDeleteKeyword.Text = '' then
    begin
      Showmessage('Please select a keyword to delete in the combo box above');
    end;
...
procedure TfrmManageKeywords.btnHelpClick(Sender: TObject);
begin
  redHelpKeywords.readOnly := True;
  redHelpkeywords.enabled  := True;
  redHelpKeywords.visible  := True;
  btnnotHelp.enabled       := True;
  btnNothelp.Visible       := True;
  pnlKeywords.hide;
  pnlDeleteKeyword.hide;
end;
...
procedure TfrmManageKeywords.btnNotHelpClick(Sender: TObject);
begin
  redHelpkeywords.enabled  := False;
  redHelpKeywords.visible  := False;
  btnnotHelp.enabled       := False;
  btnNothelp.Visible       := False;
  pnlKeywords.show;
  pnlDeleteKeyword.show;
end;
